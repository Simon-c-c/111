...
testdf4 <- testdf3[,c(1,2,3,4,5,7,8,9,10)]
...
testdf3 <- testdf3 %>% 
  mutate(code_0 = case_when(study_dur_coding < 10 ~ 0,
                            study_dur_coding > 10 ~ 1))

testdf3 <- testdf3 %>% 
  mutate(debug_0 = case_when(study_dur_debug < 10 ~ 0,
                             study_dur_debug > 10 ~ 1))
...
testdf3 <- testdf3 %>% 
  mutate(completed = case_when(get_score == 0 ~ 0,
                              get_score > 0 ~ 1))
testdf3 <- testdf3 %>% 
  group_by()
...
t2 <- testdf3 %>% 
  summarise(across(where(is.numeric), .fns= list(n = sum,
                                                 min = min, 
                                                 mean = mean,
                                                 stdev = sd,
                                                 # q25 = ~quantile(., 0.25),
